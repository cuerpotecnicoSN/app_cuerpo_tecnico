...
export type ToolType = 'select' | 'coach' | 'player' | 'cone' | 'cone-tall' | 'pole' | 'goal' | 'mini-goal' | 'goal-f11' | 'goal-f8' | 'goal-f5' | 'ball' | 'hurdle' | 'hurdle-high' | 'ring' | 'ladder' | 'bosu' | 'bosu-profile' | 'arrow' | 'dashed-arrow' | 'zone-line' | 'text' | 'shape-circle' | 'shape-square';
...
export const TaskBoardEditor: React.FC<TaskBoardEditorProps> = ({ value, onChange, initialData, readOnly, hideToolbar, printMode, rotateFullField, printWidth, limitedTools }) => {
  ...
  const getMappedCoords = useCallback((clientX: number, clientY: number) => {
  ...
  useEffect(() => {
    const handlePointerMove = (e: PointerEvent) => {
      let { x, y } = getMappedCoords(e.clientX, e.clientY);
      ...
      if (drawingLine) {
        ...
        if (corner === 'line-start') {
          ...
          const dxVal = (x - startX) * 5;
          const dyVal = (y - startY) * 5;
  ...
  }, [drawingLine, selectionBox, activeDragId, activeResizeId, activeTool, elements, selectedElementIds, getMappedCoords]);
  ...
  const renderFieldBackground = () => {
    ...
    // Se añade un margen mínimo superior e inferior de 2.5m para poder colocar elementos justos fuera del campo.
    const viewBox = (fieldType === 'full' || fieldType === 'full-horizontal')
      ? '-10 -2.5 88 110'
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox={viewBox} preserveAspectRatio="xMidYMid meet">
            <g {...lineProps}>
              ...
              <>
                ...
                <rect x="30.34" y="-0.4" width="7.32" height="1.4" fill={printMode ? 'transparent' : 'rgba(0,0,0,0.05)'} />
                ...
                <rect x="30.34" y="104" width="7.32" height="1.4" fill={printMode ? 'transparent' : 'rgba(0,0,0,0.05)'} />
            ...
            {fieldType === 'half' && (
              <>
                ...
                <rect x="30.34" y="51.5" width="7.32" height="1.4" fill={printMode ? 'transparent' : 'rgba(0,0,0,0.05)'} />
    ...
    switch (el.type) {
      ...
      case 'player':
        content = (
          <div className="relative flex items-center justify-center" style={{ width: px(28), height: px(28) }}>
            {/* Hombros */}
            <div className="absolute bg-black" style={{ width: '100%', height: px(3), bottom: px(7), borderRadius: px(1.5) }} />
            {/* Cabeza */}
            <div className="z-10 relative" style={{
              width: px(18), height: px(18), borderRadius: '50%',
            ...
            <div className={`grid ${limitedTools ? 'grid-cols-2' : 'grid-cols-2 lg:grid-cols-4 xl:grid-cols-5'} gap-1`}>
              <ToolButton tool="player" label="Jugador" bg={true} icon={
                <div className="relative flex items-center justify-center w-6 h-6">
                  ...
                  <div className="relative z-10 w-[14px] h-[14px] rounded-full border-[1.5px] border-black shadow-sm" style={{ backgroundColor: activeColor }} />
                </div>
              }/>
                ...
                <>
                  <ToolButton tool="cone" label="Chino" bg={true} icon={
                     <svg width="16" height="16" viewBox="0 0 20 20">
                       <path d="M 1 10 A 9 9 0 1 0 19 10 A 9 9 0 1 0 1 10 Z M 8 10 A 2 2 0 1 1 12 10 A 2 2 0 1 1 8 10 Z" fill={activeColor === '#ffffff' ? '#e5e7eb' : activeColor} fillRule="evenodd" />
                       <circle cx="10" cy="10" r="9" fill="none" stroke="rgba(0,0,0,0.1)" strokeWidth="0.5" />
                       <circle cx="10" cy="10" r="2" fill="none" stroke="rgba(0,0,0,0.3)" strokeWidth="0.5" />
                     </svg>
                  }/>
                  <ToolButton tool="cone-tall" label="Cono" bg={true} icon={
                     <svg width="16" height="19.2" viewBox="0 0 20 24">
                       <rect x="1" y="20" width="18" height="3" rx="1" fill={activeColor === '#ffffff' ? '#e5e7eb' : activeColor} filter="brightness(0.8)" />
                       <ellipse cx="10" cy="20" rx="7" ry="2" fill={activeColor === '#ffffff' ? '#e5e7eb' : activeColor} filter="brightness(0.9)" />
                       <path d="M 4 20 L 8 2 L 12 2 L 16 20 Z" fill={activeColor === '#ffffff' ? '#e5e7eb' : activeColor} />
                       <ellipse cx="10" cy="2" rx="2" ry="1" fill={activeColor === '#ffffff' ? '#e5e7eb' : activeColor} filter="brightness(1.2)" />
                     </svg>
                  }/>
                  <ToolButton tool="pole" label="Pica" bg={true} icon={
                    <div className="flex flex-col items-center justify-end h-5">
                      <div className="w-[2px] flex-1 rounded-t-sm" style={{ background: `linear-gradient(90deg, #fff, ${activeColor === '#ffffff' ? '#e5e7eb' : activeColor} 50%, #d1d5db)` }} />
                      <div className="w-3 h-1 rounded-t-full bg-gray-800 border-b border-gray-900" />
                    </div>
                  }/>
                  <ToolButton tool="hurdle" label="Valla Baja" bg={true} icon={<div className="w-5 h-2 border-t border-l border-r border-current" style={{ color: activeColor === '#ffffff' ? '#9ca3af' : activeColor }} />}/>
                  <ToolButton tool="hurdle-high" label="Valla Alta" bg={true} icon={<div className="w-5 h-4 border-t border-l border-r border-current" style={{ color: activeColor === '#ffffff' ? '#9ca3af' : activeColor }} />}/>
                  <ToolButton tool="ring" label="Aro" bg={true} icon={<div className="w-4 h-4 rounded-full border-2" style={{ borderColor: activeColor === '#ffffff' ? '#9ca3af' : activeColor }} />}/>
                  <ToolButton tool="ladder" label="Escalera" bg={true} icon={
                     <div className="flex flex-col gap-0.5 w-3" style={{ color: activeColor === '#ffffff' ? '#9ca3af' : activeColor }}>
                  ...
                  <ToolButton tool="bosu" label="Bosu" bg={true} icon={<div className="w-4 h-4 rounded-full border border-gray-300" style={{ background: `radial-gradient(circle at 30% 30%, ${activeColor === '#ffffff' ? '#f3f4f6' : activeColor}, #e5e7eb)` }} />}/>
                  <ToolButton tool="bosu-profile" label="Bosu Perfil" bg={true} icon={<div className="w-5 h-2.5 rounded-t-full border-b border-gray-300" style={{ background: `radial-gradient(circle at top, ${activeColor === '#ffffff' ? '#f3f4f6' : activeColor}, #e5e7eb)` }} />}/>
                </>
              )}
            </div>
          )}
      ...
      <div ref={fitContainerRef} className={`flex-1 flex items-center justify-center relative ${printMode || readOnly ? '' : 'bg-brand-black-card border border-brand-black-border rounded-xl shadow-inner p-2 sm:p-4'} overflow-hidden select-none`}>
        ...
        {!readOnly && !printMode && (
          <div className="absolute top-2 left-2 z-20 pointer-events-none flex flex-col gap-1 items-start">
             ...
             {['coach', 'player', 'cone', 'cone-tall', 'pole', 'goal', 'mini-goal', 'ball', 'hurdle', 'ring', 'ladder', 'bosu', 'text'].includes(activeTool) && <span className="bg-brand-red-600/80 text-white text-xs px-2 py-1 rounded backdrop-blur shadow-sm">🎯 Haz clic en el campo para colocar</span>}
              ...
              <>
                ...
                <div className="flex items-center gap-2 bg-black/50 rounded-lg border border-slate-700 px-3 py-1.5 w-32">
                  ...
                  <input 
                    ...
                    onChange={(e) => updateElementThickness(selectedElementIds, parseInt(e.target.value))}
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-slate-900/95 backdrop-blur-md border border-slate-700 rounded-xl p-2 flex items-center gap-2 shadow-[0_10px_30px_rgba(0,0,0,0.8)] animate-fade-in z-50 text-white">
            ...
            <div className="flex items-center gap-2 bg-black/50 rounded-lg border border-slate-700 px-3 py-1.5 w-32">
              ...
              <input 
                ...
                min="1" 
                max="20" 
                value={selectedLine.thickness || 1} 
                onChange={(e) => updateLineThickness(selectedLineId, parseInt(e.target.value))}
                className="w-full h-1 bg-gray-600 rounded-lg cursor-pointer accent-blue-500"
              />
            </div>
